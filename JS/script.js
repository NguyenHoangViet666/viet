(function() {
    ...
    function calculateBasePath() {
        ...
        if (path.includes('/TRUYEN/')) {
             // Cần kiểm tra kỹ cấu trúc thư mục con trong TRUYEN
             // Giả định độ sâu 2 cấp ../../ là phổ biến
             let depth = 2;
             const partsAfterTruyen = path.substring(path.indexOf('/TRUYEN/') + '/TRUYEN/'.length).split('/').filter(Boolean);
             if (partsAfterTruyen.length > 2) {
                depth = partsAfterTruyen.length; // Ước tính độ sâu
             }
             let basePath = '';
             for(let i = 0; i < depth; i++) { basePath += '../'; }
             return basePath;
        }
        return './'; // Mặc định là ở gốc
    }
    ...
    function isHomePage() {
        ...
        // Trường hợp 1: Gốc tuyệt đối "/"
        if (path === '/') return true;

        // Trường hợp 2: File index.html ở gốc hoặc trong thư mục repo
        if (path.endsWith('/index.html')) {
            // Nếu chỉ có "index.html" trong đường dẫn (ví dụ: /index.html)
            if (segments.length === 1 && segments[0].toLowerCase() === 'index.html') return true;
             // Nếu có dạng /repo-name/index.html
             if (segments.length === 2 && segments[1].toLowerCase() === 'index.html') return true;
        }

        // Trường hợp 3: Thư mục gốc của repo trên GitHub Pages (ví dụ /repo-name/ hoặc /repo-name)
        // Phải đảm bảo nó không phải là file khác cùng cấp (ví dụ /other-file)
        if (segments.length === 1 && !path.includes('.')) { // Không có dấu chấm -> có thể là thư mục repo
             // Kiểm tra thêm nếu nó kết thúc bằng /
             if (path.endsWith('/')) return true;
             // Nếu không kết thúc bằng / nhưng chỉ có 1 segment và không có '.', giả định là root repo
             return true;
        }


        // Mặc định không phải trang chủ
        return false;
    }
    ...
    if (navigationPlaceholder) {
        // --- QUYẾT ĐỊNH ĐƯỜNG DẪN FETCH RÕ RÀNG HƠN ---
        let navPathToFetch;
        if (IS_HOME_PAGE) {
            ...
            // Đang ở trang con (HTML/ hoặc TRUYEN/), fetch tương đối từ trang con đó
            // Đường dẫn này cần đi lên đúng số cấp (BASE_PATH) rồi vào HTML/
    ...
    function setupNavigationInteractions() {
        ...
        if (loginButtonContainer) {
             ...
             // console.log(`[BetoBook Script v5] Link paths: user=${userPagePath}, login=${loginPagePath}, home=${homePagePath}`); // Ít log hơn
             if (isLoggedIn) {
    ...
    function showSlides(n) { /* ... giữ nguyên ... */
    ...
    function startSlideShow() { /* ... giữ nguyên ... */
    ...
    function closeOtherDropdowns(currentDropdown = null) { /* ... giữ nguyên ... */
    ...
    document.addEventListener('click', function(event) { /* ... giữ nguyên ... */
